...
(defun qta/my-move-key (keymap-from keymap-to key)
  "Moves key binding from one keymap to another, deleting from the old location. "
  (define-key keymap-to key (lookup-key keymap-from key))
  (define-key keymap-from key nil))

(qta/my-move-key evil-motion-state-map evil-normal-state-map (kbd "RET"))
(qta/my-move-key evil-motion-state-map evil-normal-state-map " ")
